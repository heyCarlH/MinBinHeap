...
public class MinBinHeapTests {
	...
	public void testBuildA() {
		try {
			...
			heap.build(collectionToBuild);
			assertTrue(
					"build with empty collection doesn't keep an empty heap",
					heap.getMin() == null);
		} catch (Exception e) {
			fail("Exception thrown: " + e.getMessage());
		}
	...
	public void insertEasy() {
		...
		assertTrue("Heap created is not a min-heap",
				isMinHeap(heap.getHeap(), 1, heap.size()));
	}
	...
	public void buildLecture() {
		...
		EntryPair[] array = { new EntryPair("abc", 6),
				new EntryPair("bcd", 12), new EntryPair("cde", 9),
				new EntryPair("def", 4), new EntryPair("efg", 5),
				new EntryPair("fgh", 3), };
	...
	public void buildLargeHeapAndWreck() {
		...
		for (int i = 1; i < 9999; i++) {
			assertEquals("didn't actually delete at " + i, null,
					heap.getHeap()[i]);
		}
	}
}